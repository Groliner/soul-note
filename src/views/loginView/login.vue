...
watch(totalSteps, () => {
  ...
  refArray.forEach((el) => {
    if (responseData.value?.data) {
      ...
      el.value.value = responseData.value.data[el.value.type]
      nextStep.value = true
    } else {
      el.value.value = ''
...
const handleHover = (el) => {
  ...
  if (el.target.classList.contains('up')) inputArray.value = sigUp
  if (el.target.classList.contains('down')) inputArray.value = logIn
  isHover.value = true
...
const checkInput = (el) => {
  inputArray.value.forEach((inner, index) => {
    if (inner.type === el.value.type && index + 1 === stepNumber.value) {
...
const handleFinished = () => {
  ...
  const tweenFinish = handleAnimation()
  if (totalSteps.value > 2) handleSignUp(tweenFinish)
  else handleLogIn(tweenFinish)
}
const handleSignUp = async (tweenFinish) => {
  const data = {
...
const handleLogIn = async (tweenFinish) => {
  const data = {
    email: refArray[0].value.value,
